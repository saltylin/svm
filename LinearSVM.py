...
class LinearSVM(SVM):
    def __init__(self, trainattr, label, comid, label1, label2):
        super(LinearSVM, self).__init__(trainattr, label, comid, label1, label2)
        self.coe = 0.01
        self.T = 50 * self.num
    ...
    def training(self):
        for t in range(1, self.T + 1):
            ...
            if innerproduct * tmplabel < 1:
                self.w = (1.0 - t * coe) * self.w + yt * tmplabel * trainitem
                self.b = (1.0 - t * coe) * self.b + yt * tmplabel
            else:
                self.w = (1.0 - t * coe) * self.w
                self.b = (1.0 - t * coe) * self.b
